...
/**
 * SRP client side math.
 */
public class AuthenticationHelper {

    private BigInteger a;
    private BigInteger hashA;
    private String poolName;

    public AuthenticationHelper(final String userPoolName) {
        do {
            a = new BigInteger(EPHEMERAL_KEY_LENGTH, SECURE_RANDOM).mod(N);
            hashA = GG.modPow(a, N);
        } while (hashA.mod(N).equals(BigInteger.ZERO));
    ...
    private static final ThreadLocal<MessageDigest> THREAD_MESSAGE_DIGEST = ThreadLocal.withInitial(() -> {
        try {
            return MessageDigest.getInstance(CognitoServiceConstants.SHA256_ALGORITHM);
    ...
    static {
        try {
            SECURE_RANDOM = SecureRandom.getInstance(CognitoServiceConstants.SHA1PRNG_ALGORITHM);
    ...
    public byte[] getPasswordAuthenticationKey(final String userId,
                                               final String userPassword,
                                               final BigInteger hashB,
                                               final BigInteger salt) {
        // Authenticate the password
        // u = H(A, B)
        final MessageDigest messageDigest = THREAD_MESSAGE_DIGEST.get();
        messageDigest.reset();
        messageDigest.update(hashA.toByteArray());
        final BigInteger u = new BigInteger(1, messageDigest.digest(hashB.toByteArray()));
        ...
        final BigInteger s = (hashB.subtract(KK.multiply(GG.modPow(x, N)))
                .modPow(a.add(u.multiply(x)), N)).mod(N);

        Hkdf hkdf = null;
        try {
            hkdf = Hkdf.getInstance(CognitoServiceConstants.HMAC_SHA256_ALGORITHM);
        } catch (final NoSuchAlgorithmException e) {
            throw new SecurityException(e.getMessage(), e);
        }
